...
# Create list with numbers of paired images + Calculate and save in list fundamental matrices
num_images = 39
step = 12
imgpair_nums_list = []
T_cam_cam_list = []
...
for i in range(num_images-step):
  add_nums = [i, i+step]
  imgpair_nums_list.append(add_nums)

  for j in range(i, (i+step)):
    if j==i:
      T_cam_cam_accum = copy.deepcopy(globals()['T_cam%s'%j+'_cam%s'%(j+1)])
    else:
      T_cam_cam_accum = np.dot(T_cam_cam_accum, (globals()['T_cam%s'%j+'_cam%s'%(j+1)]))
  
  T_cam_cam_list.append(T_cam_cam_accum) 
  T_fund_list.append(fund(T_cam_cam_accum, mtx))

num_of_pairs = len(imgpair_nums_list)
imgpair_nums_np = np.array(imgpair_nums_list)
...
def SIFT_pairs(img_undist0, img_undist1, T_cam0_cam1_fund, T_cam1_cam0_fund):
  ...
  img0_sift = img_undist0.copy()
  img1_sift = img_undist1.copy()

  img_lines_0_1_sift, _0_1_sift = drawlines(img0_sift,img1_sift,epi_lines_0_sift,kp0_good,kp1_good)
  img_lines_1_0_sift, _1_0_sift = drawlines(img1_sift,img0_sift,epi_lines_1_sift,kp1_good,kp0_good)
...
def r2d2_pairs(img_undist0, img_undist1, T_cam0_cam1_fund, T_cam1_cam0_fund):
  ...
  img_undist0_r2d2_copy = copy.deepcopy(img_undist0)
  img_undist1_r2d2_copy = copy.deepcopy(img_undist1)

  img_lines_0_1_r2d2, _0_1_r2d2 = drawlines(img_undist0_r2d2_copy,img_undist1_r2d2_copy,epi_lines_0_r2d2,xys0_good,xys1_good)
  img_lines_1_0_r2d2, _1_0_r2d2 = drawlines(img_undist1_r2d2_copy,img_undist0_r2d2_copy,epi_lines_1_r2d2,xys1_good,xys0_good)
...
for i in range(num_of_pairs):
  img_n1 = imgpair_nums_np[i,0]
  img_n2 = imgpair_nums_np[i,1]
  r2d2_matches_0_1, img_lines_0_1_r2d2, img_lines_1_0_r2d2, sum_dist_r2d2 = r2d2_pairs(globals()['img_undist%s'%img_n1], globals()['img_undist%s'%img_n2],
...
#Create empty lists for storing results for different methods
...
#Process images of dataset for all pairs using all methods (except r2d2 which was calculated above in section r2d2)
for i in range(num_of_pairs):
  img_n1 = imgpair_nums_np[i,0]
  img_n2 = imgpair_nums_np[i,1]
  sift_matches_0_1, img_lines_0_1_sift, img_lines_1_0_sift, sum_dist_sift, kp0_good = SIFT_pairs(globals()['img_undist%s'%img_n1], globals()['img_undist%s'%img_n2],
                                                                                                          T_fund_list[i], T_fund_list[i].T)
  rlof_matches_0_1, img_lines_0_1_rlof, img_lines_1_0_rlof, sum_dist_rlof = RLOF_pairs(kp0_good, globals()['img_undist%s'%img_n1], globals()['img_undist%s'%img_n2],
                                                                                                          T_fund_list[i], T_fund_list[i].T)
  alike_matches_0_1, img_lines_0_1_alike, img_lines_1_0_alike, sum_dist_alike = ALIKE_pairs(globals()['img_undist%s'%img_n1], globals()['img_undist%s'%img_n2],
                                                                                                          T_fund_list[i], T_fund_list[i].T)
  sppn_matches_0_1, img_lines_0_1_sppn, img_lines_1_0_sppn, sum_dist_sppn = SPPN_pairs(globals()['img_undist%s'%img_n1], globals()['img_undist%s'%img_n2],
                                                                                                          T_fund_list[i], T_fund_list[i].T)
  orb_matches_0_1, img_lines_0_1_orb, img_lines_1_0_orb, sum_dist_orb = ORB_pairs(globals()['img_undist%s'%img_n1], globals()['img_undist%s'%img_n2],
                                                                                                          T_fund_list[i], T_fund_list[i].T)
